...
def get_router_config(config_key: str = ""):
    """Retrieve the full configuration as a dictionary.

    Args:
        config_key (str): A specific key to retrieve from the configuration. Defaults to "" (return the full configuration).
    """
    if os.path.exists(CONFIG_PATH):
        config = OmegaConf.load(CONFIG_PATH)
    else:
    ...
    if config_key:
        return OmegaConf.to_container(config, resolve=True).get(config_key)
